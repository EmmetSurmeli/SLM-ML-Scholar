...
The repository currently contains a complete character-level bigram language
model. It learns a \(V\times V\) table of next-character logits and conditions
only on the previous character. The milestone includes:
...
- pytest coverage and a mathematical implementation map

This model is **not an SLM and not yet a research-paper assistant**. It cannot
understand a paper, explain an equation, retrieve evidence, or maintain context
beyond one character. It is the deliberately small end-to-end baseline on
which later neural-network components will be tested.
...
gradient accumulation, optimizer behavior, checkpoint identity, and malformed
inputs.
...
Ruff reported no lint or formatting errors, and pytest reported `27 passed`.
...
src/localml_scholar/
  ...
  optimizers.py           Explicit SGD updates
  generation.py           Autoregressive sampling
  utils.py                Gradient checks and reporting utilities
...
- Minibatches are sampled with replacement; the trainer does not yet implement
  epochs, schedules, resume, early stopping, or gradient clipping.
- Checkpoints contain model weights, not optimizer or sampler state.
...
The next milestone implements and finite-difference-checks reusable neural
network foundations: dense layers, embeddings, activations, normalization,
momentum, Adam, and gradient clipping. A decoder-only transformer comes only
after those primitives pass independent checks. Local PDF parsing, retrieval,
licensed ML-paper specialization, evidence-based evaluation, an application,
and measured performance work follow in later milestones.
